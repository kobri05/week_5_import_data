...
youth <- read_csv("Data_RAw/YRBS2015.csv",
                  skip = 1,
                  na = c('.','N/A'),
                  col_type = cols(AgeCat = col_number(),
                                  Grade = col_number(),
                                  `Perception of weight` = col_factor(levels = 
                                                                        c("Very underweight",
                                  ...
                                  `Breakfast eating` = col_character(),
...
> Difficult variable names, a lot of N/As, and a lot of empty boxes within the data. A lot of variables have names that are either overly complicated or entirely unusable.  
...
write_csv(youth, file = 'Data_CLEAN/youth_clean.csv')
youth_clean <- read_csv('Data_CLEAN/youth_clean.csv')
```

Why do we keep the code for the data cleaning but set the code chunks to `eval: false`?

> In order to allow us to keep the data on our own and not rely on someone else to keep it for us. Additionally, including 'eval: false' makes the code more efficient so that it takes less time to load everything in and explore it.
...
> There is a lot of missing data for soda_drinking, physical_acticity_5_days, and breakfast_eating, and there is a lot of overlap in missing data for all 3 variables.  

How does the missingness of the data impact what variables you might choose to analyze? Why?

> You may not want to analyze data with lots of missing values, as the holes in the data may make your analysis less accurate.  
...
youth_clean |>  #instead of data = youth_clean in ggplot we can "pipe" our data  
  ...
  labs(x = 'Height (m)',
       y = 'Weight (kg)',
       color = 'Perception of Weight')
ggsave("weight-perception.png", device = "png")
```
